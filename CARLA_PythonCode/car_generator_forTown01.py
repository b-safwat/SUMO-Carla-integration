...
# FILE_NAME = r'..\SUMO_TrafficLog\Town01_Car50_2.log'
FILE_NAME = r'../SUMO_TrafficLog/Town01_Car50.log'
FILE_NAME = r'../SUMO_TrafficLog/Town01_Car50.log'
# FILE_NAME = r'D:\sumo\CARLA_0.9.10\WindowsNoEditor\PythonAPI\Map\Carla_map\Town01_50.log'
# FILE_NAME = r'/media/bassel/Entertainment/sumo/CARLA_0.9.10/WindowsNoEditor/PythonAPI/Map/Carla_map/Town01_50.log'
OUTPUT_PATH = 'town1/Car50/data/'
# OUTPUT_PATH = 'Car50/data/'
BSM_ORIGINALPATH = 'workplace/town1/Car50/'
SAVE_CAM_IMAGES=True

# BSM_ORIGINALPATH = 'workplace/Car50/'
...
def BSMWriter(file_path, carID_list, timestamp, actor_list, ID_table, BSM_vehicleID):
    ...
    for actor in actor_list:
        ...
        if actor.id == targetID:
            ...
            location = transform.location.x
            file_path.write(
                str(timestamp) + '\t' + str(transform.location.x) + '\t' + str(transform.location.y) + '\t' + str(
        ...
        for edge in item.findall('vehicle'):
            ...
            for vehicleID_ite in range(0, VehicleNumber):
                if collectedID == vehicleID_ite:
                    ...
                    new_vehicleInfo = {'time': collectedtime, 'id': collectedID, 'x': collectedx, 'y': collectedy,
                                       'angle': collectedangle, 'speed': collectedspeed}
                    vehicleInfo.append(new_vehicleInfo)

    return vehicleInfo


def Generate_New_Vehicles(SumoVehicleInfo_ForAll, Elapsedtime, actor_list, blueprints, world, active_carID_list,
                          killed_list, ID_table, vehicleID4Sensing):
    new_transform_list = []
    new_id_list = []

    for i in range(0, len(SumoVehicleInfo_ForAll)):
        collectedtime = SumoVehicleInfo_ForAll[i]['time']
        if (float(int(Elapsedtime)) == collectedtime and (
        not SumoVehicleInfo_ForAll[i]['id'] in active_carID_list) and (
        not SumoVehicleInfo_ForAll[i]['id'] in killed_list)):
            ...
            transform = carla.Transform(carla.Location(x=float(SumoVehicleInfo_ForAll[i]['x']),
                                                       y=344.26 - float(SumoVehicleInfo_ForAll[i]['y']) - 16, z=0.2),
                                        carla.Rotation(pitch=0, yaw=float(SumoVehicleInfo_ForAll[i]['angle']), roll=0))
            print('Generated... VehicleID is ', SumoVehicleInfo_ForAll[i]['id'], ' time is ',
                  SumoVehicleInfo_ForAll[i]['time'])
    ...
    for n, transform in enumerate(new_transform_list):
        ...
        if new_id_list[n] == vehicleID4Sensing:
            ...
            f.write(str(world.wait_for_tick().elapsed_seconds) + '\t' + str(Elapsedtime) + '\t' + str(
                c_height) + '\t' + str(x_diff_front) + '\t' + str(x_diff) + '\t' + str(x_diff_rear) + '\t' + str(
                y_diff))
            f.close()
            ...
            if SAVE_CAM_IMAGES:
                ...
                # camera_bp.set_attribute("fps", str(10.0))
                ...
                camera.listen(lambda image: image.save_to_disk(
                    OUTPUT_PATH + str(new_id_list[n]) + '/front/rgb_%s.png' % (image.timestamp)))
                ...
                camera = world.spawn_actor(camera_bp, camera_transform, attach_to=actor)
                camera.listen(lambda image: image.save_to_disk(
                    OUTPUT_PATH + str(new_id_list[n]) + '/front_Right60/rgb_%s.png' % (image.timestamp)))
                transform = camera.get_transform()
                transform = actor.get_transform()
                actor_list.append(camera)
                ...
                camera = world.spawn_actor(camera_bp, camera_transform, attach_to=actor)
                camera.listen(lambda image: image.save_to_disk(
                    OUTPUT_PATH + str(new_id_list[n]) + '/front_Left300/rgb_%s.png' % (image.timestamp)))
                transform = camera.get_transform()
                transform = actor.get_transform()
                actor_list.append(camera)
                ...
                camera = world.spawn_actor(camera_bp, camera_transform, attach_to=actor)
                camera.listen(lambda image: image.save_to_disk(
                    OUTPUT_PATH + str(new_id_list[n]) + '/frontD/depth_%s.png' % (image.timestamp), carla.ColorConverter.Depth))
                transform = camera.get_transform()
                transform = actor.get_transform()
                actor_list.append(camera)
                ...
                camera = world.spawn_actor(camera_bp, camera_transform, attach_to=actor)
                camera.listen(lambda image: image.save_to_disk(
                    OUTPUT_PATH + str(new_id_list[n]) + '/front_Right60D/depth_%s.png' % (image.timestamp), carla.ColorConverter.Depth))
                transform = camera.get_transform()
                transform = actor.get_transform()
                actor_list.append(camera)
                ...
                camera = world.spawn_actor(camera_bp, camera_transform, attach_to=actor)
                camera.listen(lambda image: image.save_to_disk(
                    OUTPUT_PATH + str(new_id_list[n]) + '/front_Left300D/depth_%s.png' % (image.timestamp), carla.ColorConverter.Depth))
                transform = camera.get_transform()
                transform = actor.get_transform()
                actor_list.append(camera)
...
def ChangeLocation4ExistingCars(SumoVehicleInfo_ForAll, Previous_Elapsedtime, Elapsedtime, actor_list, actor_carID_list,
                                VehicleNumber, killed_list, ID_table):
    counter = 0
        ...
        for i in range(0, len(SumoVehicleInfo_ForAll)):
            ...
            if (targetID == SumoVehicleInfo_ForAll[i]['id'] and Floor_time == SumoVehicleInfo_ForAll[i]['time']):
                ...
                if flag_ceil == False:
                    ...
                    print('shunsuke: ', iterate_number, ' and i is ', i, 'and,', Ceil_time, targetID, killed_list,
                          Elapsedtime)
                    print(' ')
                if (flag_ceil == True):
...
def main():
    ...
    SumoVehicleInfo_ForAll = parseXML_forAllVehicles(FILE_NAME, VehicleNumber)
    try:
    ...
    try:
        ...
        (actor_list, active_carID_list, ID_table) = Generate_New_Vehicles(SumoVehicleInfo_ForAll, elapsedtime,
                                                                          actor_list, blueprints, world,
                                                                          active_carID_list, killed_list, ID_table,
                                                                          args.vehicleID)
        ...
        while True:
            ...
            if previous_mill_sec > elapsedtime_mill_sec:
                ...
                (actor_list, killed_list, active_carID_list) = Kill_Finished_Vehicles(SumoVehicleInfo_ForAll,
                                                                                      elapsedtime, actor_list,
                                                                                      killed_list, active_carID_list,
                                                                                      ID_table)
                ...
                (actor_list, active_carID_list, ID_table) = Generate_New_Vehicles(SumoVehicleInfo_ForAll, elapsedtime,
                                                                                  actor_list, blueprints, world,
                                                                                  active_carID_list, killed_list,
                                                                                  ID_table, args.vehicleID)
                ...
                if elapsedtime > 500:
                    ErrorPath = '/home/usrp/carla/PythonAPI/shunsuke_workplace/OUTPUT_PATH' + str(
                        args.vehicleID) + '.txt'
            ...
            Active_Car_Num = ChangeLocation4ExistingCars(SumoVehicleInfo_ForAll, previous_elapsedtime,
                                                         elapsedtime, actor_list, active_carID_list, VehicleNumber,
                                                         killed_list, ID_table)

            BSMWriter(bsm_f, active_carID_list, timestamp.elapsed_seconds, actor_list, ID_table, args.vehicleID)
...
if __name__ == '__main__':
    ...
    town, car = scenario.split("_")[0], scenario.split("_")[1].split(".")[0]

    FILE_NAME = r'../SUMO_TrafficLog/'+scenario
    OUTPUT_PATH = town+'/' + car + '/data/'
    BSM_ORIGINALPATH = 'workplace/'+town+'/' +car + '/'
